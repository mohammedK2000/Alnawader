...
class aboutauss extends StatefulWidget {
  
 
 
  @override
 aboutaussState createState() =>aboutaussState( );
}

 

class aboutaussState extends State<aboutauss> {
...
@override
          ...
          body: Column(
            ...
            child: Padding(
              ...
              child: Container(width: 30,height: 30,child: Center(child: Icon(Icons.arrow_forward,color: Colors.white,),),),
                ...
                child: Image.asset('assets/information.png',color: Colors.white,),
                ),
                ),
                  Positioned(top: 120,
                left: maxwidth/2-60,
                right: maxwidth/2-60,
                child:Center(child: Text('عن التطبيق',style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold),))
                ),
               ],),
               ),
  ...
  var responsess = await http.get(
      Uri.parse(baseurl + "about_application"), headers: header
    );
